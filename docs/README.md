...
* 主要なゲームロジック・探索・学習コードは `alphazero/` パッケージに集約しました。以下のモジュールが含まれています。
  * `alphazero/game.py`: 盤面表現や合法手列挙などのゲーム状態クラス。
  * `alphazero/mcts.py`: ニューラルネット推論を用いるモンテカルロ木探索の実装。
  * `alphazero/network.py`: ポリシー／バリューヘッドを備えたニューラルネットワーク定義。
  * `alphazero/training.py`: 自己対戦データの前処理、学習ループ、評価ユーティリティ。
  * `alphazero/random_mcts.py`: 乱数ロールアウト専用の軽量 MCTS 実装。
* `alphazero/__init__.py` で上記モジュールの代表的なクラスや関数を再公開しているため、アプリ側では `from alphazero import State, Tree` のように一貫したインポートができます。
* `app/` ディレクトリはユーザー向けのエントリーポイントを提供します。アーティファクト管理や学習ループ、棋譜ビューアなどの機能はここに位置づけ、ドメインロジックと分離しました。
* 既存のテストコードは新しいパッケージ構成を利用するよう更新済みです。追加テストを書く際も `alphazero` パッケージ経由で必要なクラスを参照してください。
