...
* 石が増えた局面では石を包含する矩形領域へ候補手を制限し、終盤でも無駄に全盤面を走査しないよう最適化しました。
* ロールアウト中は単純な一様分布で合法手を選択し、深手のランダム探索でも 1 手あたりの処理時間を大幅に短縮しています。
...
* パス時の必勝受け判定は合法手の総数だけでなく、既存の石の近傍にある候補手数を基準に切り替えるようになり、序盤でも局所的な危険が見つかった場合は確実に受けを探しにいきます。候補が広がり過ぎる場合は旧来どおり探索を抑制し、1 手ごとに数百手を総当たりする状況を避けます。
* connect6 の防御判定では縦横斜めの線分から空点と石数を差分解析し、開放四や片開き五など同一ターンで完結する脅威を即座に抽出します。合法手が数百残る序盤でも局所的な受け漏れが発生しません。
